...
foot_dr <- layers_aggregate(drivers, by = "both")
foot_vc <- layers_aggregate(vc, by = "both")
...
dat <- layers_extract(expo, drivers = dr_sel, vcs = vc_sel) 
...
# Cumulative effects of each driver across all vc
dat <- layers_aggregate(halpern, by = "vcs")
...
dat <- layers_aggregate(halpern, by = "drivers") 
plot(merge(dat), breaks = "equal", col = pal)
```
...
dat <- layers_aggregate(dat, by = "both") 
...
dat <- layers_aggregate(beauchesne$net, by = "both") 
plot(dat, breaks = "equal", col = pal)
```
...
dat <- layers_aggregate(beauchesne$direct, by = "both") 
plot(dat, breaks = "equal", col = pal)
```
...
dat <- layers_aggregate(beauchesne$indirect, by = "both") 
plot(dat, breaks = "equal", col = pal)
```
